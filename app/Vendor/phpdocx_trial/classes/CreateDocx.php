...
class CreateDocx extends CreateDocument {
    ...
    public static $log;
    public static $sectionProperties = array('w:footnotePr', 'w:endnotePr', 'w:type', 'w:pgSz', 'w:pgMar', 'w:paperSrc', 'w:pgBorders', 'w:lnNumType', 'w:pgNumType', 'w:cols', 'w:formProt', 'w:vAlign', 'w:noEndnote', 'w:titlePg', 'w:textDirection', 'w:bidi', 'w:rtlGutter', 'w:docGrid', 'w:printerSettings', 'w:sectPrChange');
    ...
    public function __construct($baseTemplatePath = PHPDOCX_BASE_TEMPLATE) {
        ...
        $this->_defaultPHPDOCXStyles = array('Default Paragraph Font PHPDOCX', 'List Paragraph PHPDOCX', 'Title PHPDOCX', 'Subtitle PHPDOCX', 'Normal Table PHPDOCX', 'Table Grid PHPDOCX', 'footnote Text PHPDOCX', 'footnote text Car PHPDOCX', 'footnote Reference PHPDOCX', 'endnote Text PHPDOCX', 'endnote text Car PHPDOCX', 'endnote Reference PHPDOCX', 'annotation reference PHPDOCX', 'annotation text PHPDOCX', 'Comment Text Char PHPDOCX', 'annotation subject PHPDOCX', 'Comment Subject Char PHPDOCX', 'Balloon Text PHPDOCX', 'Balloon Text Char PHPDOCX');
        ...
        if ($this->_defaultTemplate) {
            ...
            for ($j = $counter; $j > -1; $j--) {
                ...
                $arrayCleaner = array();
                switch ($type) {
                    case 'header': array_push($this->_relsHeader, $target);
                        break;
                    case 'footer': array_push($this->_relsFooter, $target);
                        break;
                    case 'chart': $this->recursiveDelete($this->_baseTemplateFilesPath . '/word/charts');
                        $this->_wordRelsDocumentRelsT->documentElement->removeChild($relationships->item($j));
                        break;
                    case 'embeddings': $this->recursiveDelete($this->_baseTemplateFilesPath . '/word/embeddings');
                        $this->_wordRelsDocumentRelsT->documentElement->removeChild($relationships->item($j));
                        break;
                }
  ...
  //      $this->_wordSettingsT->documentElement->appendChild($settingsImage);
        ...
        try {
            if (isset($options['data']) && isset($options['type'])) {
                ...
                if (strpos($type, 'Chart') === false)
                    $type .= 'Chart'; $graphic = CreateChartFactory::createObject($type);
    ...
    public function addElement($type, $params = '') {
        ...
        if ($type == 'CreateGraphic') {
            if (strpos($params['type'], 'Chart') === false)
                $params['type'] .= 'Chart'; $element = CreateChartFactory::createObject($params['type']);
        } else {
                    ...
                    if ($attrImage['mime'] == 'image/jpg' || $attrImage['mime'] == 'image/jpeg' || $attrImage['mime'] == 'image/png' || $attrImage['mime'] == 'image/gif') {
                        ...
                        if ((string) $image != '') {
                            if ($data['target'] == 'defaultHeader' || $data['target'] == 'firstHeader' || $data['target'] == 'evenHeader' || $data['target'] == 'defaultFooter' || $data['target'] == 'firstFooter' || $data['target'] == 'evenFooter') {
                                $this->_relsHeaderFooterImage[$data['target']][] = array('rId' => 'rId' . self::$intIdWord, 'extension' => $dir['extension']);
                            } else if ($data['target'] == 'footnote' || $data['target'] == 'endnote' || $data['target'] == 'comment') {
                                $this->_relsNotesImage[$data['target']][] = array('rId' => 'rId' . self::$intIdWord, 'extension' => $dir['extension']);
                            } else {
    ...
    public function addMergeField($name, $mergeParameters = array(), $options = array('rawWordML' => false)) {
        ...
        foreach ($mergeParameters as $key => $value) {
            switch ($key) {
                case 'textBefore': $data .= '\b &quot;' . $value . '&quot; ';
                    break;
                case 'textAfter': $data .= '\f &quot;' . $value . '&quot; ';
                    break;
                case 'mappedField': if ($value) {
                        $data .= '\m ';
                    } break;
                case 'verticalFormat': if ($value) {
                        $data .= '\v ';
                    } break;
                case 'preserveFormat': if ($value) {
                        $data .= '\* MERGEFORMAT';
                    } break;
            }
...
/*            $this->addRawWordML($message);
            $this->addBackgroundImage(dirname(__FILE__) . '/../examples/files/img/imagebg.jpg');
    ...
    public function parseStyles($path = '') {
        ...
        $myTable = array(array('Title A', 'Title B', 'Title C'), array('First row A', 'First row B', 'First row C'), array('Second row A', 'Second row B', 'Second row C'));
        ...
        foreach ($parsedNodes as $node) {
            ...
            $wordMLChunk = '<w:tbl><w:tblPr><w:tblW w:w="0" w:type="auto"/>
                ...
                ';
            switch ($styleType) {
                case 'table': $wordMLChunk = str_replace('CODEX', "addTable(array(array('Title A','Title B','Title C'),array('First row A','First row B','First row C'),array('Second row A','Second row B','Second row C')), array('TBLSTYLEval'=> '$styleId'))", $wordMLChunk);
                    ...
                    break;
                case 'paragraph': $myPCode = "addText('This is some sample paragraph test', array('pStyle' => '" . $styleId . "'))";
    ...
    private function addDefaultFootnote() {
        ...
        $footnote = CreateFootnote::getInstance();
        $footnote->createInitFootnote(array('type' => 'continuationSeparator'));
        $this->_wordFootnotesC .= (string) $footnote;
    }
    ...
    private function generateDefaultFonts() {
        $font = array('name' => 'Calibri', 'pitch' => 'variable', 'usb0' => 'A00002EF', 'usb1' => '4000207B', 'usb2' => '00000000', 'usb3' => '00000000', 'csb0' => '0000009F', 'csb1' => '00000000', 'family' => 'swiss', 'charset' => '00', 'panose1' => '020F0502020204030204');
        $this->addFont($font);
        $font = array('name' => 'Times New Roman', 'pitch' => 'variable', 'usb0' => 'E0002AEF', 'usb1' => 'C0007841', 'usb2' => '00000009', 'usb3' => '00000000', 'csb0' => '000001FF', 'csb1' => '00000000', 'family' => 'roman', 'charset' => '00', 'panose1' => '02020603050405020304');
        $this->addFont($font);
        $font = array('name' => 'Cambria', 'pitch' => 'variable', 'usb0' => 'A00002EF', 'usb1' => '4000004B', 'usb2' => '00000000', 'usb3' => '00000000', 'csb0' => '0000009F', 'csb1' => '00000000', 'family' => 'roman', 'charset' => '00', 'panose1' => '02040503050406030204');
        $this->addFont($font);
    }
    ...
    private function HTMLRels($sFinalDocX, $options) {
        ...
        if ($options['target'] == 'defaultHeader' || $options['target'] == 'firstHeader' || $options['target'] == 'evenHeader' || $options['target'] == 'defaultFooter' || $options['target'] == 'firstFooter' || $options['target'] == 'evenFooter') {
            ...
            foreach ($sFinalDocX[1] as $key => $value) {
                $this->_relsNotesLink[$options['target']][] = array('rId' => $key, 'url' => $value);
            }
        } else {
            foreach ($sFinalDocX[1] as $key => $value) {
    ...
    public function importHeadersAndFooters($path, $type = 'headerAndFooter') {
        switch ($type) {
            case 'headerAndFooter': $this->removeHeadersAndFooters();
                break;
            case 'header': $this->removeHeaders();
                break;
            case 'footer': $this->removeFooters();
                break;
        } try {
        ...
        for ($j = $counter; $j > -1; $j--) {
            ...
            switch ($myType) {
                case 'header': $relsHeader[$rId] = $target;
                    break;
                case 'footer': $relsFooter[$rId] = $target;
                    break;
            }
            ...
            foreach ($relsHeader as $key => $value) {
                if (file_exists($baseHeadersFootersFilesPath . '/word/_rels/' . $value . '.rels')) {
                    ...
                    for ($j = $countrels; $j > -1; $j--) {
                        ...
                        $myType = array_pop(explode('/', $completeType));
                        switch ($myType) {
                            case 'image': $refExtension = array_pop(explode('.', $target));
