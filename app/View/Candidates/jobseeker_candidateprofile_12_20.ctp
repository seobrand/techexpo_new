...
$(document).ready(function(){ 
	...
	$(function() {
		...
		//$("#worktype").draggable({"scroll":false});
   ...
   //	 });
		...
		$("#worktype ul").draggable({
                disabled : false
            });
			
		
		
		$("#resumeID ul").draggable({
                disabled : false
            });
			
		
		$("#upcomingEvents ul").draggable({
                disabled : false
            });
			 ...
			 if($SeekAnn['HomepageMessage']['msg']){ ?>
              <div class="announcements_panel">
                ...
                <ul class="dash_act_job">
                  <li><?php echo $SeekAnn['HomepageMessage']['msg']; ?> <br/>
                    <?php 
						if(!empty($SeekAnn['HomepageMessage']['img']) && file_exists('img/homepage/'.$SeekAnn['HomepageMessage']['img']))
						 echo $this->Html->image("homepage/".$SeekAnn['HomepageMessage']['img'],array('style'=>'max-width:600px'));
						else 
				  ...
				  if(count($resumeRec))
				  {
				   foreach($resumeRec as $value){ ?> 
                    <tr class="dash_act_job with_action">
                        ...
                        <td><span class="f_left jobname"><?php echo $value['Resume']['resume_title'];?></span></td>
