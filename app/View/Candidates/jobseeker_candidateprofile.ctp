      ...
      <div class="whiteB_mid border_mid">
        <div class="whiteB_bottom border_btm">
          <div class="content">
               ...
               <?php if($value9['records']['recordID']=='1') {?>
                  <li style="width:290px;float:left;margin:10px;min-height:370px;margin:5px 5px 0px 15px;" id="recordsArray_<?php echo $value9['records']['id'];; ?>">
                    ...
                    <div class="dash_ev_list" >
                    	<?php if(count($showRec)){ ?>
                        <ul class="dash_ev_list sidebar_upcoming upcoming_event_dashboard">
						<?php foreach($showRec as $value){  ?>                        	
							<li>
								<p>
                            <span> 
                            <a href="<?php echo $this->Html->url(array('controller'=>'shows','action'=>'view',$value['Show']['id'],'Jobseeker'=>false)) ; ?>" >
                            <?php if(!empty($value['ShowsHome']['display_name'])) echo $value['ShowsHome']['display_name']; else  echo $value['Show']['show_name']; ?>
                            </a>
                            </span>
                                 <br/>   
                                   <?php echo $value['Location']['location_city'].','.$value['Location']['location_state'];?><br/>
			
            <?php if(!empty($value['ShowsHome']['special_message'])) echo $value['ShowsHome']['special_message']; else { ?>
        
        	 Security Clearance <?php     if($value['Show']['sec_clearance_req']=='n') echo "<strong>Not</strong>";  ?> REQUIRED</p>
             <?php } ?>   
								</p>
								...
								<?php 
									$eventValReg = $common->isCandidateRegForEvent($this->Session->read('Auth.Clients.candidate_id'),$value['Show']['id']);
									if($eventValReg==0)
									echo $this->Html->link('Register',array('controller'=>'shows','action'=>'eventRegister',$value['Show']['id']),array('escape'=>false,'class'=>'viewMore','style'=>'float:right;text-decoration:none !important;')); 
									else
									echo $this->Html->image('registered.png',array('class'=>'viewMore','style'=>'float:right')); 
								?>
							</li>							
                        	<?php }?>
						 </ul>
              ...
              <li style="width:290px;float:left;margin:10px;min-height:370px;margin:5px 5px 0px 15px;" id="recordsArray_<?php echo $value9['records']['id'];; ?>">
              	 <h4 class="dash_heading">Your Resumes</h4>
                <table class="resume_tbl">
                	<tr  class="active_job_heading">
                    	<th class="jobname">Resume Title</th>
            ...
            <li style="width:290px;float:left;margin:10px;min-height:370px;margin:5px 5px 0px 15px;" id="recordsArray_<?php echo $value9['records']['id'];; ?>">
            <?php // echo $this->Html->link($this->Html->image("ads/bannerad_canddash.png"), array(''),array('class'=>'f_right','escape'=>false)); ?>
			<?php $adBanner = $common->adBanner(2);
            if(!empty( $adBanner)) {
            ?>
            <a href="<?php echo $adBanner['Banner']['href']; ?>" <?php if($adBanner['Banner']['link_type']=='external'){?>target="_blank"<?php }?>> <img src="<?php echo $this->webroot; ?>thumbnail.php?file=<?php echo 'Banner/'.$adBanner['Banner']['filename'];?>&maxw=300&maxh=200"  /> </a>
          	 <?php } ?>
              <!--<div class="dashboard_col_lf">
                <h4 class="dash_heading">Videos</h4>
                <?php
                if($dashboard_video['CandidateVideo']['id']){
