<?php echo $this->element('admin-breadcrumbs',array('pageName'=>'editrole')); ?>
<?php echo $this->Form->create('Role', array('action'=>'update'));?>
<div class="title-pad">
  <div class="title">
  ...
  <div class="table">
    <table border="0" cellspacing="0" cellpadding="0" id="login" align="center">
      <tr>
        ...
        <td width="85%" align="left" valign="top"><?php echo $this->Form->input('role_name', array('label'=>'','id'=>'role_name','class'=>'inputbox1','tabindex'=>1,'div'=>false,'error'=>false,'value'=>$data['Role']['role_name'],'style'=>'width:192px;height:15px;')); ?></td>
      </tr>
     
      <?php if($data['Role']['id'] != 1) {//starting permissions..... ?>
      <?php 
	 if(!isset($userpermissions) || is_array($userpermissions) ) { $userpermissions = array();}
      ...
      <?php //$defaultArr = array('adminusers/home','adminusers/index','adminclients/index','candidates/index','usertimesheets/index','Adminjobs/index','Areaofinterests/index','ethnicities/index','Jobtypes/index','Joblocations/index','salaries/index','titles/index','pages/index','adminusers/index','roles/index','settings/index','email_templates/index'); 
		if(isset($data['Role']['allowedfunctions']) && $data['Role']['allowedfunctions']) {
			$defaultArr = explode(',',$data['Role']['allowedfunctions']);
		}
		else {
		 $defaultArr = array(); 
		}
		?>
        <td align="left" valign="top"><?php echo '&nbsp;&nbsp;'.$this->Form->checkbox('main',array('onClick' => 'makeSelectedCheckboxs("RoleAllowedfunctions",this.checked,"counterId")')); ?>&nbsp;Select All</td>
      </tr>
      <tr>
      ...
      <?php $permissionArr = array(				
		...
		foreach($permissionArr as $title=>$permission) {
			 ...
			 foreach($permission as $key=>$val)  {
			 	 ...
			 	 echo '&nbsp;&nbsp;';
				 echo $this->Form->input('Role.allowedfunctions.'.$i,array('label'=>'','type'=>'checkbox','value'=>$key,'checked'=>in_array($key,$defaultArr),'div'=>false));
		 ...
		 echo $this->Form->input('counterId',array('type'=>'hidden','value'=>$i,'id'=>'counterId'));
		?>
    <?php } }?>	 
      <tr>
        <td align="right" valign="top">&nbsp;</td>
        <td align="left" valign="top"><?php echo $this->Form->input('id',array('label'=>'','type'=>'hidden','id'=>'userroleid','value'=>$id,'div'=>false)); ?>
