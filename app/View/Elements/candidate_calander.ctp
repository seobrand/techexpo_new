<?php  
	  ...
	  echo $form->input($dateN, array('label'=>$label,'id'=>$dateId,'div'=>false,'value'=>$defaultDate,'style'=>$style,'class'=>'input_01','onblur'=>'convert_date()','onclick'=>'if(this.value=="DD-MM-YYYY") {this.value="";}'));
		?>
		<?php echo $html->image('calendar_icon.jpg',array('alt'=>'Cal','align'=>'absmiddle','onclick'=>"displayCalendar(document.getElementById('".$dateId."'),'".CALENDAR_DATE_FORMAT."',this)",'div'=>false,'onmouseover'=>'delete_date();','onmouseover'=>'add_date()','onmouseout'=>'clear_date()'));		
?>
<script type="text/javascript">

var validformat=/^\d{2}\-\d{2}\-\d{4}$/;
function add_date() {
	if((document.getElementById('<?php echo $dateId;?>').value!='01-01-1995' || document.getElementById('<?php echo $dateId;?>').value=='DD-MM-YYYY') && !validformat.test(document.getElementById('<?php echo $dateId;?>').value)) {
		document.getElementById('<?php echo $dateId;?>').style.color = 'white';
		document.getElementById('<?php echo $dateId;?>').value= '01-01-1995';
	}
}
function clear_date() {
