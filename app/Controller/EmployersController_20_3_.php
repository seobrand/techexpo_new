...
class EmployersController extends AppController {
	...
	function profile() {
		...
		if($this->request->is('post')){
			...
			{	
				...
				$this->request->data['Employer']['employer_name']=$this->request->data['Employer']['employer_fname'].' '.$this->request->data['Employer']['employer_lname'];
				...
				if($this->Employer->save($this->request->data)){
						...
						$bodyText1 = 'Nancy / Brad,<br/><br/>An employer has created a new profile on techexpoUSA.com with the following details:<br/><br/>Company Name : '.$this->request->data['Employer']['employer_name'].'<br/>Contact Email : '.$this->request->data['EmployerContact']['contact_email'].'<br/>Phone : '.$this->request->data['Employer']['main_phone'].' <br/> on date '.date("F d, Y",time()).'.For more detail login to admin section.';
						...
						$popupmsg = 'Thank you for registering.   All companies and job postings are screened in advance. A TECHEXPO representative will contact you shortly.  You may contact us directly at 1-212-665-4505 ext. 224 or email us at admin@techexpousa.com';
	...
	*******************************************************************************************/
			...
			if(!$this->EmployerContact->validates()){
				...
				if(trim($this->request->data['User']['password'])!=$this->request->data['User']['currentpassword']){
					if(trim($this->request->data['User']['password'])!=$this->request->data['User']['confirmpassword']) {
						...
						$data = array('User' => array('id'=>$this->request->data['User']['id'],'password'=>$this->request->data['User']['password']));
	...
	function empeditjob($postingID = null){
		...
		//	pr($this->request->data);
			...
			if(!$errorsArr) 
			{
				...
				$keywordCount = $this->request->data['JobPosting']['count'];
				$this->request->data['JobPostingSkill']['posting_id'] = $postingID;
	...
	function joblists(){
		...
		// Get the list of login employer job
		$condition  = 'JobPosting.employer_id = '.$employerID.' AND JobPosting.employer_contact_id =  '.$empContactID;	
		$this->paginate = array('limit' =>20,'order' => 'JobPosting.posting_id DESC');
		$joblists = $this->paginate('JobPosting', $condition);
		$this->set('joblists',$joblists);
		// Get totalcount of jobs
		$jobcount = $this->JobPosting->find("count",array('fields' => 'DISTINCT JobPosting.posting_id','conditions'=>$condition));
	...
	function empevent(){
		...
		if($this->request->is('get')){
			...
			$condition  = "Show.show_dt >= '".$targetdate."' ";	
			$this->Show->recursive =0;
			$this->paginate = array('limit' =>20,'order' => array('show_dt ASC'));
	...
	public function empVideo($action = null,$id = null  )
	{
		...
		{	
			...
			{	
					$name = time().'_'.$this->data['Employer']['video']['name']; // move file
					move_uploaded_file( $this->data['Employer']['video']['tmp_name'], "upload/video/employer/" .$name);
			...
			// mail to admin
						$sendto = 'brand@techexpousa.com,pprobert@techexpousa.com,kfuller@techexpousa.com';
						$sendfrom = $this->Session->read('Auth.Client.User.candidate_email');
	...
	public function empJobseekerVideo(){
		...
		//$extracted = Set::extract($events, '/ShowEmployer/show_id');
		...
		$jobseekervideo_list = $this->CandidateVideo->find('all',array('conditions'=>' CandidateVideo.candidate_id in (\''.$candidatestring.'\') '));
	...
	{ 
		...
		{
			$this->Auth->allow('profile','jobdetail','emailjob','eventregistrationform','recruitattechexpo','empexhibitor','empshowVideo','city','viewprofile','empcotactcity');
		}
