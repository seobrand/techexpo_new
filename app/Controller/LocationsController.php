 ...
 */
	public function superadmin_index() { 
        ...
        $this->paginate = array('limit' =>$record,'order' => array('Location.location_state' => 'ASC'));
 ...
 */
	public function superadmin_add() {
		...
		public function superadmin_add2() {
		...
		if($this->request->is('post')) {		
			...
			if($errors) {			
				...
				if ($this->Location->save($this->request->data)) {
					$this->Session->write('popup','Location has been added successfully.');
					$this->Session->setFlash('Location has been added successfully.');  
					$this->redirect(array('controller'=>'shows','action' => "add/message:success"));
				}else{
					$this->Session->setFlash('Data save problem, Please try again.');  
                }					
			}	
		...
		if($this->request->is('post')) {		
			...
			echo "<option selected='selected' value='".$LocationId."'>".$this->request->data['Location']['location_state']." ".$this->request->data['Location']['site_name']."</option>";die;
 ...
 */
	...
	public function superadmin_copytraveldirection() {
		...
		$shows = $this->Show->find('all',array('fields'=>'location_id,show_travel_dir','conditions'=>array('Show.show_travel_dir !=' => '')));
		pr($shows);
	}
