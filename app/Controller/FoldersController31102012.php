...
class FoldersController extends AppController {
	...
	{ 
			...
			if(!empty($this->params->query['cleareance_list']))
				...
				$conditions.= "and (
									(security_clearance_code!='3650')
									and (security_clearance_code not like '%,3650')
									and (security_clearance_code not like '3650,%')
									and (security_clearance_code not like '%,3650,%')
								)";

			$conditions.="and (
								(security_clearance_code!='3922')
								and (security_clearance_code not like '%,3922')
								and (security_clearance_code not like '3922,%')
								and (security_clearance_code not like '%,3922,%')
							)";	
		...
		//	echo $conditions;die;
				...
				$this->set('listCount', $this->Resume->find('count',array('conditions'=>$conditions)));
				}else
				{ 
	...
	{ 
			...
			if(!empty($this->params->query['selected_db'])) 
			{
				...
				if($this->params->query['selected_db']=='all')
				{
					
					$setLists = $this->ResumeSet->query('select resume_id from resume_sets');
				}else
				{
					
					$setLists = $this->ResumeSet->query('select resume_id from resume_sets where set_id="'.$this->params->query['selected_db'].'"');
				}
			
		
				
				$totalResumeSet=count($setLists);
				
				$ResumeIds='';
				$i=1;
				foreach($setLists as $values)
				{
					$ResumeIds.=$values['resume_sets']['resume_id'];
					if($i!=$totalResumeSet)
					{
						$ResumeIds.=',';
					}
				...
				if($ResumeIds)
				{
					$ResumeIds = str_replace(',','\',\'',$ResumeIds);
					$conditions="Resume.id in('".$ResumeIds."')";
				}
			}
			else
			{
				...
				if(!empty($this->params->query['state_list']))  // search condition for state list
					{
							if(!empty($conditions))
							{
								$conditions.=' and ';
					...
					if(!empty($this->params->query['keyword'])) // search for citizen ship status
					{
							if(!empty($conditions))
							{
								$conditions.=' and ';
				  ...
				  if(!empty($this->params->query['cleareance_list']))
							{
										...
										if(is_array($this->params->query['cleareance_list']))
										{
											$securityClearances=$this->params->query['cleareance_list'];
											count($securityClearances);
											$totalSecurityClearance=count($securityClearances);
											...
											foreach($securityClearances as $key7=>$value7)
											{
												$conditions.= 'Candidate.security_clearance_code like "%'.$value7.'%"';
			...
			$conditions.="and (
			 ...
			 $this->paginate = array('fields'=>'Candidate.candidate_name,Candidate.id,Candidate.candidate_title,Candidate.candidate_email,Candidate.experience_code,
			 									Candidate.citizen_status_code,Candidate.security_clearance_code,Candidate.candidate_state,Candidate.candidate_city,
												Resume.resume_title,Resume.id,Resume.resume_summary,Resume.posted_dt
			 									',
			 							'conditions'=>$conditions,
										'limit' => 10,
										'paramType' => 'querystring',
										'order'   => 'Resume.posted_dt DESC'
										);
