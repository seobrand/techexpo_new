...
**********************************************************************<br>";
	...
	//		$email->from(array($sendfrom));
			$email->from('webmaster@TechExpoUSA.com');
		...
		$this->Session->write('search_reg_url',$_SERVER[ 'REQUEST_URI' ]);
			...
			if($this->request->isGet())
			{
				...
				if(count($this->params->query))			// if query string found
				{
					...
					if($resume_result>0){
						$this->paginate = array(
								'fields' => array("distinct(Resume.id),Resume.resume_title,Resume.source_code,Resume.resume_summary,Resume.posted_dt,Resume.resume_content,Resume.filename,Candidate.*"),
