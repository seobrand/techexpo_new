...
class FoldersController extends AppController {
	...
	function scheduleInteview($candidate_id = null,$sendmail = null)
	{
		...
		$this->Employer->id = $employerID ;
		$Ename= $this->Employer->field('employer_name');
		...
		$conditions2 =  " Show.show_dt > '".$targetdate."' "; // apurav condidtion 07092013
		if(!isset($already_schedule))
		{
			
			if($PostinIdList)
			{
				$conditions2.=' AND Show.id in ('.$PostinIdList.') ';
			}
...
Good News ! ".$Ename." would like to set up a time to interview you at the event. This will upgrade your registration status to VIP. When you get to the show, DO NOT wait in line. Simply print out this e-mail and bring it with you directly to the event's registration desk. You will be escorted into the event by a TECHEXPO member without having to wait in line<br><br>

You can view and print a list of all the companies who wish to interview you at the event from the website located on your dashboard in the event registration center at ".BASE_URL."/jobseeker_register - Additionally, an event guide including the list of companies will be printed and waiting for you at the event's registration desk.
...
**********************************************************************<br>";
	...
	function showResume($resume_id = null)
	{
		...
		/************** Entry in resume_access_stat table**************/
		$this->common->resumeAccessStat($this->Session->read('Auth.Client.employerContact.employer_id'),$resume_id,'v');
		/****************************/
	...
	function downloadResume($filename)
	{
			 ...
			 $this->downloadFile('candidateDocument/resume',$filename);
		
	}

	function updatenotes($id = null)
	{
		$foldername = $this->FolderContent->find('first',array('conditions'=>array('FolderContent.fc_id'=>$id),'fields'=>array('notes','fc_id','folder_id','edited_by','edit_date')));
		...
		if($this->request->isPost())
		{
			...
			$this->request->data['FolderContent']['edited_by'] = $this->data['Folder']['edited_by'];
			$this->request->data['FolderContent']['edit_date'] = date('Y-m-d');
			$result = $this->FolderContent->save($this->request->data);
	...
	public function mailResume($resume_id = null ,$action = null)
	{
		...
		if(!$errorsArr && $action=='sendresume')
		{
			...
			$subject = "Resume forwarded-Posted on techexpoUSA.com";
			$bodyText = "This resume was forwarded to you by one of your colleagues using the resume database on   <a href='http://techexpousa.com' >TechExpoUSA.com </a><br/><br/>*************************************************************************************<br/>".$emailMessage."<br/><br/> Thank You <br> Tech Expo Team ";
	...
	//		$email->from(array($sendfrom));
			$email->from(FROM_MAILING_ADDRESS);
			$email->to($sendto);
			$email->emailFormat('html');
		//	$email->setHeaders(array('MIME-Version'=>'1.0','Content-type'=>'multipart/mixed; charset="iso-8859-1'));
		//	$email->replyTo( 'webmaster@mailinglist.techexpomail.com');
		//	$email->returnPath( 'webmaster@mailinglist.techexpomail.com');
			$email->subject(strip_tags($subject));
			//var_dump($email->send($bodyText));die;
			$ok = $email->send($bodyText);
			
			if($ok){
				$this->Session->write('popup','Resume Sent');
	...
	function searchRegCandidate()
	{
		...
		$condition  = "ShowEmployer.employer_id = ".$employerID." AND Show.id IS NOT NULL ";
		...
		$targetdate = date("Y-m-d",mktime(0, 0, 0, date("m") , date("d"), date("Y")));
		$condition2  = "ShowEmployer.employer_id = ".$employerID." AND Show.id IS NOT NULL AND Show.show_dt >= '$targetdate' ";
		$regEventsshow = $this->ShowEmployer->find('all',array('conditions'=>$condition2,'order'=>array('Show.show_dt DESC')));
		$this->set('regEventsshow',$regEventsshow);	
	...
	function searchRegResult($action = null)
	{
		...
		// code added 01-11-2013
		$this->loadModel('ShowEmployer');
		$event_list = $this->ShowEmployer->find('all',array('conditions'=>array('ShowEmployer.employer_id'=>$this->Session->read('Auth.Client.employerContact.employer_id'),'Show.id !='=>''),'fields' => array('Show.id'),'order'=>array('Show.show_dt DESC')));
		...
		$this->Session->write('search_reg_url',$_SERVER[ 'REQUEST_URI' ]);
			...
			if($this->request->isGet())
			{
				...
				if(count($this->params->query))			// if query string found
				{
					...
					$this->Session->write('search_reg_word',$this->params->query['words']);
					$this->Session->delete('advancSrchKeywordSh');
					...
					// add usa and canada condition
			
				if (in_array("USA", $this->params->query['candidate_state'])) {
						...
						$stateList =$this->State->find('list',array('fields'=>array('id','state_abrev'),'conditions'=>'State.state_abrev!="00" and State.country="usa"','order'=>array('state_name')));
						 $candidate_state = array_unique(array_merge($candidate_state,$stateList));
						 $this->params->query['candidate_state']= array_unique(array_merge($candidate_state,$stateList));
					
				}
				if (in_array("Canada", $this->params->query['candidate_state'])) {
						$this->loadModel('State');
					...
					if($search_type=='Any'){
						...
						$stlist = 1;
						if(isset($this->params->query['candidate_state']) && is_array($this->params->query['candidate_state'])){
					...
					// or replace with and conditions task id 2804
 if(($this->params->query['citizen_status_code']!="") AND ($this->params->query['citizen_status_code']!='0')){
						...
						{  // conditions add apurav 01-11-2013
											
							$showList = 1;
							$conditions .= " AND (";
							foreach($event_list as $showID){
								if($showList==1){
									$conditions .= "(Registration.show_id=".$showID['Show']['id'].")";
								}else{
									$conditions .= " OR (Registration.show_id=".$showID['Show']['id'].")";
								}
								$showList++;
							}
							$conditions .= ")";
						...
						$this->Session->write('advancSrchKeywordSh',$matchesWD);
					...
					// task id 2803
						...
						$matches[] = $words;
						$this->Session->write('advancSrchKeywordSh',$matches);
					...
					if($search_type=='Advanced'){
						...
						// condition added for add with (  apurav 22 jan 2014 (Tssk id 3092)
						if(stripos(trim($words), " or ")===false and stripos(trim($words), " not ")===false)
						{
							$words	=	str_ireplace("(", "", trim($words));
							$words	=	str_ireplace(")", "", trim($words));
						}
						// end 
						...
						if(strlen($words)>0){
							...
							$this->Session->write('advancSrchKeywordSh',$matches[1]);
							if(strlen($word)>0){
					...
					// candidate name or condition added task id 2888
					$conditions .= " AND Resume.source_code >= 0 ";
					
					if(!empty($search_words))
					$conditions .= " OR Candidate.candidate_name like '%".$search_words."%' ";
					...
					//pr($this->Resume->getLastQuery());die;
					...
					// word count and sort condition start
					$highlightkey = $this->Session->read('advancSrchKeywordSh');
					//pr($highlightkey);die;
					if(is_array($highlightkey) and count($highlightkey) > 0 )
					{ 
						//$wordsAll = explode(' ',$words);
						$conditionPlus='';
						$totalWrd = 0; 
						foreach($highlightkey as $word)
						{
							if($totalWrd!=0) $conditionPlus.=" + "; 	
							$conditionPlus.= " ((char_length(resume_content) - char_length(replace(LOWER(resume_content),LOWER('".$word."'),''))) / char_length('".$word."')) ";	
							$totalWrd++;	
						}	
						
						if(!empty($conditionPlus))
						{
							$this->Resume->virtualFields['SearchCount'] = $conditionPlus;
							$extraField = " SearchCount ";	
						}	
						else
						{
							$extraField = "";
					...
					// word count and sort condition end
					$this->Resume->recursive = 1;
					if($resume_result>0){
						$this->paginate = array(
								'fields' => array("distinct(Resume.id)","Resume.*"/*,"Resume.resume_title","Resume.source_code","Resume.resume_summary","Resume.posted_dt","Resume.resume_content","Resume.filename"*/,"Candidate.* "),
								'conditions' => array('(Resume.id IN ('.$reslist.'))'),
								'limit' => '10',
								'order' => array('SearchCount' =>'DESC'),
								'contain' =>array('Resume','Candidate')
						);							
	...
	{ 
		...
		$this->set('resumeLists',array());
		// Get the list of database which is assign to login employer employer 19 september 2013
		...
		// added new line for all assign condition 19 september 2013
		$assignAllResumeSetIds = Set::extract($selected_db, '{n}.ResumeSetRule.set_id');
		...
		if($this->request->isGet())
		{
			...
			if(count($this->params->query))			// if query string found
			{
				...
				// add line 22 july 2013
				$this->Session->write('search_db_word',$this->params->query['words']);
					...
					if(is_array($this->params->query['security_clearance_code2']) && count($this->params->query['security_clearance_code2'])>0){
						...
						foreach($all_sec_clr as $sec_clrs){
							if(in_array($sec_clrs['ClearanceKeyword']['clearance_code_id'],$this->params->query['security_clearance_code2']) && $baselist!='38'){
								...
								// added
								if(count($this->params->query['security_clearance_code2']) && in_array('38',$this->params->query['security_clearance_code2']))
								{
								$complex_search_string=$complex_search_string.' (';	
								}
						...
						//echo $complex_search_string;die;
						$security_clearance_code3	=	$complex_search_string;
					}
				}
				...
				if($search_type=='Any'){
					...
					// condition added 19 september 2013				
					if(is_array($set_id) && $set_id[0]=='all'){
						$resume_set_ids = implode(",", $assignAllResumeSetIds);
						$conditions .= " AND ResumeSet.set_id IN (".$resume_set_ids.")";
					}	
					
					$matchesWD = explode(' ',$words);	
					$this->Session->write('advancSrchKeyword',$matchesWD);
				...
				if($search_type=='Exact Phrase'){
					...
					$matches[] = $words;
					$this->Session->write('advancSrchKeyword',$matches);
				...
				if($search_type=='Advanced'){ 
					...
					if(strlen($words)>0){
						...
						if($pos===false){ 
						 ...
						 preg_match_all('/\"(.+?)\"/', $word, $matches);
						$this->Session->write('advancSrchKeyword',$matches[1]);
						if(strlen($word)>0){
					//		echo $word.'<br/>';
					//		$word = str_ireplace("or"," OR Resume.resume_content like ",$word);
					//		$word = " Resume.resume_content like ".$word;
							$match_condition .=	"(MATCH(Resume.resume_content) AGAINST('(".$word.")' IN BOOLEAN MODE))";
							$match_condition .= ")";
					//		echo $match_condition.'<br/>';
 ...
 if(($this->params->query['citizen_status_code']!="") AND ($this->params->query['citizen_status_code']!='0')){
				...
				// word count and sort condition start
					$highlightkey = $this->Session->read('advancSrchKeyword');
					if(is_array($highlightkey) and count($highlightkey) > 0 )
					{ 
						//$wordsAll = explode(' ',$words);
						$conditionPlus='';
				...
				$this->Resume->recursive = 1;
				if($resume_result>0){
					//Resume.resume_title,Resume.source_code,Resume.resume_summary,Resume.posted_dt,Resume.resume_content,Resume.filename
					$this->paginate = array(
							'fields' => array("distinct(Resume.id),Resume.*,Candidate.*"),
							'conditions' => array('(Resume.id IN ('.$reslist.'))'),
							'limit' => '10',
							'order' => array('SearchCount' =>'DESC'),
							'contain' =>array('Resume','Candidate')
