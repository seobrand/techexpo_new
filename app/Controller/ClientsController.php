...
class ClientsController extends AppController {
	...
	public function superadmin_clientsearch(){
		...
		$record = ($this->Session->read('per_page_record')) ? $this->Session->read('per_page_record') : ADMIN_PER_PAGE_RECORD;
			...
			if($Submit == 'Save Letter Selection'){
				...
				$this->paginate = array('limit' =>$record,'fields'=>array('Employer.id','Employer.logo_file','Employer.employer_name','Employer.city','Employer.state','EmployerContact.id','EmployerContact.contact_name','EmployerContact.contact_email'),'order' => array('Employer.id'=>'DESC'));
				...
				$this->Employer->recursive = 0;
				$this->paginate = array('limit' =>$record,'fields'=>array('Employer.id','Employer.logo_file','Employer.employer_name','Employer.city','Employer.state','EmployerContact.id','EmployerContact.contact_name','EmployerContact.contact_email'),'order' => array('id DESC'));
				$employers = $this->paginate('Employer', $condition);
				$this->set('employers',$employers);			
	...
	public function superadmin_assignevent($employerID=null){ 
		...
		if($this->request->is('get')){
			...
			// published conditions added  1632 ( 30 Aug 2013 )
			$condition = "Show.show_dt > '".$cutoff_date."' AND Show.published=1 ";
			$this->Show->recursive = 0;
			$eventList = $this->Show->find("all",array('fields'=>array('Show.id','Show.show_dt','Show.show_confirm_file','Show.boutique','Location.location_city','Location.location_state'),'order'=>array('Show.show_dt DESC'),'conditions'=>$condition));
						...
						if($this->request->data['Show']['exhibitorType'][$showID]=='R'){
							...
							if($this->request->data['Show']['emailSend'][$showID]=='Y'){
								if($do_mail==1){
									...
									$bodyText = 'Dear TECHEXPO exhibitor,<br/><br/>
									Thank you for participating in '.$this->request->data['Show']['location_state'][$showID].'s TECHEXPO on '.date("F d, Y",strtotime($this->request->data['Show']['show_dt'][$showID])).'.You are now currently assigned to the show on our website. Attached to this e-mail is your confirmation packet. Please print this document out for your review as it contains *VERY IMPORTANT* information. <br/><br/>

If you have not done so already, please take a moment to complete your employer profile located in your dashboard. Once that is completed, you will have access to post your 25 jobs at http://www.TechExpoUSA.com/users/login<br/><br/>

Why is this important? Technology professionals can view your company name and logo which will link directly to your employer profile. Your employer profile will show candidates your open positions that you have posted. Candidates can apply directly online, sending resumes your way via e-mail. You can then get in touch with candidates via e-mail to schedule show day interviews in advance.<br/><br/>

Your employer profile is also where the website pulls your company information and open positions for the event guide, which goes to print one week prior to the event. A completed profile also gives your company a more professional image and will increase traffic at your booth on the day of the show.<br/><br/>

Thank you in advance for your cooperation.<br/><br/>

By following the link above you will automatically log into the website and it will  take you to your employer homepage.<br/><br/>

Here is your TECHEXPO login information:<br/><br/>

-Log in from the main homepage at http://www.TechExpoUSA.com<br/>
-Your Username: '.$employerContact['User']['username'].' <br/>
-Your Password: '.$employerContact['User']['old_password'].' <br/><br/>
...
212.655.4505 ext. 225';
							...
							$this->ShowEmployer->save($showEmployerData);
							//check whether mail sent IS YES
							if($this->request->data['Show']['emailSend'][$showID]=='Y' && $this->request->data['Show']['boutique'][$showID]!='b'){
								if($do_mail==1){
									$content .='E-Mail sent';
									// send email to employer
									$attachfilePath = WWW_ROOT."ShowsDocument/showConfirmFile/".$showConfirmFile;
...
-Log in from the main homepage at http://www.TechExpoUSA.com<br/>
-Your Username: '.$employerContact['User']['username'].' <br/>
-Your Password: '.$employerContact['User']['old_password'].' 

<br/><br/>If you have any questions or need technical assistance, email nmathew@TechExpoUSA.com or call Nancy Mathew at 212.655.4505 ext. 225. <br/><br/>
Sincerely, <br/>
Nancy Mathew<br/>
Events Director<br/>
212.655.4505 ext. 225';
							...
							if($this->request->data['Show']['emailSend'][$showID]=='Y'){
								if($do_mail==1){
									...
									$bodyText = 'Dear TechExpoUSA.com client,<br/><br/>
Thank you for participating in our virtual job fair at our '.$this->request->data['Show']['location_state'][$showID].'s TECHEXPO on '.date("F d, Y",strtotime($this->request->data['Show']['show_dt'][$showID])).'. You have just been assigned to the show as a virtual exhibitor on our website.<br/><br/>

The virtual job fair includes:<br/><br/>
- Your company logo uploaded through your employer page displayed on the show information page as a virtual exhibitor<br/>
- The ability to post 25 jobs on our site<br/>
- Access to the resume database for that event including both resumes collected at the show and those submitted online.<br/>
- Inclusion In the event guide, which will be passed out on the day of the show. (You must fill out your company profile to be included)<br/><br/>

If you have not done so yet, please take a moment to complete your profile and post your jobs at <br/>http://www.TechExpoUSA.com/users/login<br/><br/>

Why is this important? Technology professionals can view your company name and logo which will link directly to your employer profile. Your employer profile will show candidates your open positions that you have posted. Candidates can apply directly online, sending resumes your way via e-mail. You can then get in touch with candidates via e-mail to schedule show day interviews in advance.<br/><br/>
 
Your company profile is also where the website pulls your company information and open positions for the event guide, which goes to print one week prior to the event. A completed profile also gives your company a more professional image and will increase traffic at your booth on the day of the show. <br/><br/>

 If you have purchased the Virtual Job Fair to an event that has not yet happened, use the "Pre-Reg Resumes" tool located on your dashboard.  If the event has already passed, please use the "search resumes" tool.<br/><br/>

 Following the link included above will automatically log you in and take you to your employer home page.<br/><br/>

Here is your TECHEXPO log in information :<br/><br/>
...
If you have not done so yet, please take a moment to complete your profile and post your jobs at http://www.TechExpoUSA.com/users/login<br/><br/>
...
-Log in from the main homepage at http://www.TechExpoUSA.com<br/><br/>
-Your Username:  '.$employerContact['User']['username'].' <br/>
-Your Password: '.$employerContact['User']['old_password'].' <br/><br/>
If you have any questions or need technical assistance, email nmathew@TechExpoUSA.com or call Nancy Mathew at 212.655.4505 ext. 225.  <br/><br/>
...
212.655.4505 ext. 225';
	...
	public function superadmin_assignresumedb($employerID=null,$sendEmail=null){ 
		...
		if($this->request->is('post')){
			...
			// task id 3283
			$existing_resume_set = $this->EmployerSet->find('all',array('fields'=>array('EmployerSet.set_id'),'conditions'=>array('EmployerSet.employer_id'=>$employerID)));
			$existing_resume_set = Set::extract("{n}/EmployerSet/set_id",$existing_resume_set);	
			// new assigned resume set
			$new_resume_set = $this->request->data['ResumeSetRule']['isAssign'];
			
			// get difference of resume set (whether assigned or unassigned) task #3283
			if(count($new_resume_set)>count($existing_resume_set)){
				$assigned .= 'assigned';
				$final_resume_set = array_diff($new_resume_set,$existing_resume_set);
			}else{
				$assigned .= 'unassigned';
				$final_resume_set = array_diff($existing_resume_set,$new_resume_set);
			}
			...
			foreach ($final_resume_set as $resumeSetID){
			...
			if($do_mail=='y'){
				...
				$bodyText = 'Dear TECHEXPO Client,<br/><br/>
Your resume access privileges have just been updated. You can check which databases you currently have access to from your "employer profile" which you can locate by clicking on the "Dashboard." Visit us at http://www.TechExpoUSA.com to begin searching and post up to 25 jobs.<br/><br/>

As a reminder, here is your login information:<br/><br/>

-username: '.$employerContact['User']['username'].'<br/>
-password: '.$employerContact['User']['old_password'].'<br/><br/>

****** ONLINE FEATURES ******<br/><br/>

-KEYWORD HIGHLIGHTS: when conducting a resume keyword search, searched keywords are highlighted.<br/><br/>
-RESUME FOLDERS. Create resume folders, then file resumes on the fly, copy them from folder to folder, download them... making the organization of resumes around themes, keywords or specific job titles really simple.  <br/><br/>

To search resumes and post jobs, simply go to http://www.TechExpoUSA.com and log in.<br/><br/>
...
212.655.4505 ext. 225';
				...
				if(is_array($sendtoemails)){
					foreach($sendtoemails as $sendTo){
						if(Validation::email($sendTo)){
							$email->to($sendTo);
							$email->send($bodyText);  // uncomment later 2952
						}
					}
				}
			...
			// emails get from admin system variables
			$admin_emails = $this->SystemVariable->find('first',array('conditions'=>array('SystemVariable.id'=>1)));
			$admin_email = explode(",",$admin_emails['SystemVariable']['variable_value']);
			//should be uncomment later
			if(is_array($admin_email)){
				foreach($admin_email as $sendTo){
					if(Validation::email($sendTo)){
						$email->to($sendTo);
						$email->send($bodyText1);
					}
	...
	public function superadmin_editcompanyprofile($employerID=null){
		...
		if($this->request->is('get')){
			...
			// temp code
			if(strlen($this->request->data['Employer']['main_phone'])==13)
			$this->request->data['Employer']['main_phone'] = substr($this->request->data['Employer']['main_phone'],0,-1);
	...
	public function superadmin_editcompanycontact($employerID=null, $employerContactID=null){
		...
		if($this->request->is('get')){
			...
			$companyRcd = $this->EmployerContact->find("first", array('conditions' => array('EmployerContact.id'=>$empContactID)));
			$empcontact_email = $companyRcd['EmployerContact']['contact_email'];
			$this->set('empcontact_email',$empcontact_email);
			$this->request->data = $companyRcd;
			...
			$this->request->data['User']['newusername'] = $this->request->data['User']['username'];
					...
					if($this->request->data['EmployerContact']['contact_email']==$this->request->data['EmployerContact']['contact_email_old'])
					unset($this->request->data['EmployerContact']['contact_email']);
					$this->User->set($this->request->data);
					$this->EmployerContact->set($this->request->data);
					
					$this->set('empcontact_email',$this->request->data['EmployerContact']['contact_email_old']);
					...
					if(!$errorsArr){
						...
						// insert in employer
						$this->Employer->read(null, $employerID);
						$this->Employer->set('myprofile', $this->request->data['Employer']['myprofile']);
						$this->Employer->save();
...
You can log in at http://www.TechExpoUSA.com<br/><br/>
Username: '.$employerContact['User']['username'].'<br/>
Password: '.$employerContact['User']['old_password'].'<br/><br/>
...
212.655.4505 ext. 225';
			...
			if(strpos($employerContact['EmployerContact']['contact_email'],',')){
				$sendtoemails = explode(",",$employerContact['EmployerContact']['contact_email']);
				$sendtoemails = array_filter(array_map('trim', $sendtoemails));
			}else{
				$sendtoemails=(array)$employerContact['EmployerContact']['contact_email'];
				$sendtoemails = array_filter(array_map('trim', $sendtoemails));
			}
			...
			//****** Should be uncomment later
			//echo '<pre>';
			//print_r($sendtoemails);die;
			
			
			$email->to($sendtoemails);
			$email->send($bodyText);
		...
		if($this->request->is('get')){
			...
			if($this->request->data['Submit']=='View / Refresh  Exhibitors' || $this->request->data['Submit']=='Update'){
				...
				if($resumeSet['Show']['resume_set_id']!=''){
					...
					$employerList = $this->ShowEmployer->find("all",array('conditions'=>array('ShowEmployer.show_id'=>$showID),'order'=>'Employer.employer_name ASC'));
				...
				foreach($employerList as $key => $employer){ 
					...
					if($isAssigned==0){
						if($employer['ShowEmployer']['payment_status']=='y'){
							...
							$bodyText = 'Dear TECHEXPO Client,<br/><br/>
Your resume access privileges have just been updated. You can check which databases you currently have access to on your "employer profile." Visit us at http://www.TechExpoUSA.com to begin searching and post up to 25 jobs.<br/><br/>

Here is your TECHEXPO account information:<br/>
-username: '.$employerContact['User']['username'].'<br/>
-password: '.$employerContact['User']['old_password'].'<br/><br/>

****** ONLINE FEATURES ******<br/><br/>
-KEYWORD HIGHLIGHTS: when conducting a resume keyword search, searched keywords are highlighted.<br/>
-RESUME FOLDERS. Create resume folders, then file resumes on the fly, copy them from folder to folder, download them...making the organization of resumes around themes, keywords or specific job titles really simple. <br/><br/>
...
<br/>212.655.4505 ext. 225';
	...
	function superadmin_sendemailconfirm($showID=null,$employerID=null){
		...
		if(strlen($showConfirmFile)>0){
			...
			$bodyText = 'Dear TECHEXPO exhibitor,<br/><br/>
Thank you for participating in '.$Show['Location']['location_state'].'s TECHEXPO on '.date("F d, Y",strtotime($Show['Show']['show_dt'])).'. You have just been assigned to the show on our website. Attached to this e-mail is your confirmation packet. Please print it out and review it carefully as it contains *VERY IMPORTANT* information.<br/><br/>

If you have not done so already, please take a moment to complete your employer profile and post your 25 jobs at http://www.TechExpoUSA.com/emphomeautologin/'.$employerID.'/'.$employerContact['EmployerContact']['id'].'/'.$employerContact['User']['status_code'].'<br/><br/>
	...
	function superadmin_createtrailaccount(){
		...
		if($this->request->is('post')){	
			...
			if($this->request->data['TrialAccount']['start_time']['meridian']=='am'){
				if($this->request->data['TrialAccount']['start_time']['hour']==12){
					$this->request->data['TrialAccount']['start_time']['hour'] = "00";
				}
			...
			if($this->request->data['TrialAccount']['end_time']['meridian']=='am'){
				if($this->request->data['TrialAccount']['end_time']['hour']==12){
					$this->request->data['TrialAccount']['end_time']['hour'] = "00";
				}
			...
			if(!$errorsArr){
				...
				$subject1 = "TechExpo : Trial Account Created";
				$email->emailFormat('html');
				$email->subject(strip_tags($subject1));
