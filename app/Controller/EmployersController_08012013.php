	...
	*******************************************************************************************/
	...
	function empeventprep(){
		...
		if($this->request->is('get')){
			...
			$this->paginate = array('limit' =>20,'order' => 'Show.show_dt DESC');
	...
	function empevent(){
		...
		if($this->request->is('get')){
			...
			$this->paginate = array('limit' =>20,'order' => 'show_dt DESC');
	...
	public function empJobseekerVideo(){
		...
		$employerID = $this->Session->read('Auth.Client.employerContact.employer_id');
		$targetdate = date("Y-m-d",mktime(0, 0, 0, date("m")-1 , date("d"), date("Y")));
	echo	$condition  = "ShowEmployer.employer_id = ".$employerID." AND ShowEmployer.payment_status = 'y' AND Show.show_dt > ".$targetdate." ";	
	...
	//	print_r($events);
		...
		$jobseekervideo_list = $this->CandidateVideo->find('all',array('conditions'=>' CandidateVideo.candidate_id in (\''.$candidatestring.'\') ','fields'=>array('CandidateVideo.*','Candidate.id','Candidate.candidate_name')));
