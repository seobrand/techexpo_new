...
class CandidatesController extends AppController {
	...
	//pr($this->Session->check('Auth.Client'));
		...
		$stateList=$this->common->getStateList(); //    find statelist
		$this->set('statList',$stateList);
		
		$experienceArray=$this->common->getExperienceList();  // find experince array
		$this->set('experienceArray',$experienceArray);
		
		
		$citizenshipArray=$this->common->getCitizenShipList(); // find citizenship array
		$this->set('citizenshipArray',$citizenshipArray);
		
		$govenmentclearanceArray=$this->common->getGovCleareanceList(); // find Govenment Clearance array
		$this->set('govenmentclearanceArray',$govenmentclearanceArray);
		
		$noticeperidArray=$this->common->getnoticeperiodList(); // find Govenment Clearance array
		$this->set('noticeperidArray',$noticeperidArray);
		
		$industriesArray=$this->common->getIndustriesList(); // find Industries Type
		$this->set('industriesArray',$industriesArray);
									...
									$codeIds=$this->Code->find('all',array('fields'=>array('code_id'),'order'=>array('code_id'),'conditions'=>array('code_name'=>'Skills','visible'=>'Y','code_descr like "%'.$value12.'%"'),'order'=>array("sort_order ASC")));
										...
										if($stringSkills)
										{
											$postingIds=$this->JobPostingSkill->find('all',array('fields'=>'posting_id',
																								'conditions'=>'JobPostingSkill.skill_id in ('.$stringSkills.')'));
											$totalPostingIds=count($postingIds);
											$stringPostingIds='';
											$j=1;
						...
						if(!empty($this->params->query['security_clearance_code']))
							{
										...
										if(is_array($this->params->query['security_clearance_code']))
										{
										 ...
										 {
												$conditions.= 'FIND_IN_SET("'.$this->params->query['security_clearance_code'].'",JobPosting.security_clearance_code)';
