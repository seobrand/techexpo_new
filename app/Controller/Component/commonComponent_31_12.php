   ...
   /* Functioon to get all subcates ids */
	...
	return $arr;
		...
		function getStateList() 
		{
			...
			return $this->State->find('list',array('fields'=>array('state_abrev','state_name'),'conditions'=>'State.state_abrev!="00"','order'=>array('state_name')));
		}
		...
		function getShowList()  // get upcoming show list
		{
			...
			$this->Show = new Show(); 
			return $this->Show->find('list',array('fields'=>array('id','showsNameDate'),'order'=>array('show_dt asc'),'conditions'=>array('show_dt>="'.date(DATE_FORMAT,time()).'"')));
		}
		// get show name
		...
		function getPreiousShowList()  // get previous show list
		{
			...
			return $this->Show->find('list',array('fields'=>array('id','show_name','show_name','show_dt'),'order'=>array('id'),
															'conditions'=>array('show_dt<="'.date(DATE_FORMAT,time()).'"'),
															'order'=>'show_dt desc'
															));
		}
		...
		function getprofileImage() 
		{
			...
			$candidateRec=$this->Candidate->find('first',array('fields'=>'Candidate.candidate_image',
																		'conditions'=>'Candidate.id="'.$this->Session->read('Auth.Client.User.candidate_id').'"'));
					
					
			return $candidateRec['Candidate']['candidate_image'];
		}
		...
		function regularPreRegistration($candidate_id = null,$show_id =null)
		{
			...
			return $this->ShowInterview->find("count", array('conditions'=>$conditions,'fields'=>array('ShowInterview.candidate_id')));
	...
	function upcomingevent()
	{
			APP::import('Model','Show');
			$this->Show = new Show();		
			$targetdate = date(DATE_FORMAT,mktime(0, 0, 0, date("m") , date("d"), date("Y")));
			$condition  = "Show.show_dt >= '".$targetdate."' ";	
			return $this->Show->find("all",array('conditions'=> $condition,'limit' =>'3','order' => array('show_dt ASC')));
	...
	// function for showing employer site pages usage history 
		...
		function showBanner(){
			...
			$banenrs = $this->Banner->find('all',array('conditions'=>array('Banner.category_link !='=>'4'),'order' => 'rand()'));
			return $banenrs;
		}
		...
		function getHomePageBanner(){
			APP::import('Model','Banner');
			$this->Banner = new Banner();
			$banenrs = $this->Banner->find('first',array('conditions' => array('Banner.category_link'=>'4')),array('order'=>'Banner.id DESC'));
			return $banenrs;
		}
