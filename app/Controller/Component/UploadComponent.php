<?php
	...
	class UploadComponent extends Object {
					...
					if (is_uploaded_file($fileTmp)) {
						...
						if ($rules == NULL) {
							...
							if (move_uploaded_file($fileTmp, $output)) {
								chmod($output, 0644);
								$this->imageurl = basename($this->_name);
							} else {
		...
		function image ($file, $type, $size,$output,$quality) {
                   //pr($file);echo $type;pr($size);echo $output." ".$quality;
                    //echo $file.$type.$size.$output.$quality;
