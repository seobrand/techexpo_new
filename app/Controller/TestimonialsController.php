...
public function superadmin_recruitwithus() {
		$this->set('meta_title','Recruit With Us');
		$errors = "";
		
		if($this->request->is('get')){
			$this->request->data = $this->PageContent->findById('5');			
		}else{
			$this->PageContent->set($this->request->data);
			if(!$this->PageContent->validates()){
				$errors = $this->PageContent->validationErrors;
			...
			if($errors) {
				...
				if ($this->PageContent->save($this->request->data)){
					$this->Session->write('popup','Recruit With Us page has been changed successfully.');
					$this->Session->setFlash('Recruit With Us page has been changed successfully.');
					$this->redirect(array('controller'=>'testimonials','action' => "recruitwithus/message:success"));
				}else{
					$this->Session->setFlash(__('Recruit With Us page could not be changed. Please, try again.'));
				}
			}	
		}
	}
...
public function superadmin_jobposting() {
		$this->set('meta_title','Job Posting');
		$errors = "";
		
		if($this->request->is('get')){
			$this->request->data = $this->PageContent->findById('6');			
		}else{
			$this->PageContent->set($this->request->data);
			if(!$this->PageContent->validates()){
				$errors = $this->PageContent->validationErrors;
			...
			if($errors) {
				...
				if ($this->PageContent->save($this->request->data)){
					$this->Session->write('popup','Job Posting page has been changed successfully.');
					$this->Session->setFlash('Job Posting page has been changed successfully.');
					$this->redirect(array('controller'=>'testimonials','action' => "jobposting/message:success"));
				}else{
					$this->Session->setFlash(__('Job Posting page could not be changed. Please, try again.'));
				}
			}	
		}
	}	
