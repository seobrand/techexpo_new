			...
			//pr($this->request->data['ShowsHome']);
				...
				//  $this->ShowsHome->create();
				  if(is_array($dataCheck)){
					...
					if(!empty($dataCheck['c']) && $dataCheck['c']!='0')  
					$dataCheck['assign_homepage']=1;
					else if($dataCheck['c']=='0')  
				  	$dataCheck['assign_homepage']=0;
				...
				if($ok){
					  $this->Session->write('popup','Shows Home has been created successfully.');
					  $this->Session->setFlash('Shows Home has been created successfully.');  
					  $this->redirect(array('controller'=>'ShowsHomes','action' => "index/message:success"));
				}
}
	}
